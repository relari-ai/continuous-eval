...
pytest.importorskip("torch", reason="Torch is required for the tests.")
pytest.importorskip("pandas", reason="Pandas is required for the tests.")
...
def test_answer_relevance():
    ...
    x = metric.batch(**list_of_dicts_to_dict_of_lists(data))
    assert all(
        z["bert_answer_relevance"] > 0 and z["bert_answer_relevance"] < 1
        for z in x
    )
...
def test_answer_similarity():
    ...
    y = metric(**dataset[1])
    assert (
        abs(x[1]["bert_answer_similarity"] - y["bert_answer_similarity"]) < 1e-1
    )
...
def test_deberta_answer_scores():
    ...
    y = metric(**data[0])
    assert (
        abs(x[0]["deberta_answer_entailment"] - y["deberta_answer_entailment"])
        < 1e-5
    )
    assert (
        abs(
            x[0]["deberta_answer_contradiction"]
            - y["deberta_answer_contradiction"]
        )
        < 1e-5
    )
